...
describe('Player Management', () => {
  ...
  test('should add a new player successfully', async () => {
    ...
    // Fill out the form
    await userEvent.type(document.querySelector('[data-testid="player-name-input"]'), 'Test Player', window);
    await userEvent.select(document.querySelector('[data-testid="player-position-select"]'), 'midfield', window);
    await userEvent.type(document.querySelector('[data-testid="player-team-input"]'), 'TST', window);
    await userEvent.type(document.querySelector('[data-testid="player-price-input"]'), '8.5', window);
    await userEvent.select(document.querySelector('[data-testid="player-status-select"]'), 'green', window);
    ...
    expect(fplManager.players.length).toBe(1);
    expect(fplManager.players[0].name).toBe('Test Player');
    expect(fplManager.players[0].position).toBe('midfield');
    expect(fplManager.players[0].team).toBe('TST');
    expect(fplManager.players[0].price).toBe(8.5);
    expect(fplManager.players[0].status).toBe('green');
  });
  ...
  test('should edit an existing player', async () => {
    ...
    fplManager.players.push(testPlayer);
    await fplManager.updateDisplay();
    
    // Open the edit modal
    const editButton = document.querySelector(`[data-testid="edit-player-${testPlayer.id}"]`);
    ...
    // Check that the player was updated
    expect(fplManager.players[0].name).toBe('Original Name Updated');
  });
  ...
  test('should delete a player', async () => {
    ...
    const deleteButton = document.querySelector(`[data-testid="delete-player-${testPlayer.id}"]`);
