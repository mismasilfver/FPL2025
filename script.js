...
class UIManager {
    ...
    initElements() {
        ...
        // Alert container (created if missing)
        this.alertEl = document.querySelector('[data-testid="app-alert"]');
        if (!this.alertEl) {
            this.alertEl = document.createElement('div');
            this.alertEl.setAttribute('data-testid', 'app-alert');
            this.alertEl.setAttribute('role', 'alert');
            this.alertEl.style.display = 'none';
            this.alertEl.className = 'app-alert';
...
// ===== Centralized alert helpers =====
UIManager.prototype.showAlert = function(message, { timeout = 5000 } = {}) {
    if (!this.alertEl) {
        // Fallback: ensure initElements creates it
        this.initElements();
    }
    if (!this.alertEl) return; // as a last resort, avoid throwing in tests
    ...
    // Render content with a simple close affordance
    const safeMsg = String(message || '');
    this.alertEl.innerHTML = `<span class="app-alert__msg"></span><button class="app-alert__close" aria-label="Close alert" title="Close">×</button>`;
    const msgEl = this.alertEl.querySelector('.app-alert__msg');
    if (msgEl) msgEl.textContent = safeMsg;
    // Ensure on top and visible
    this.alertEl.style.display = 'block';
    // Inline styles to guarantee visibility even if CSS isn't loaded
    this.alertEl.style.position = 'fixed';
    this.alertEl.style.top = '12px';
    this.alertEl.style.left = '50%';
    this.alertEl.style.transform = 'translateX(-50%)';
    this.alertEl.style.maxWidth = '90%';
    this.alertEl.style.zIndex = '9999';
    this.alertEl.style.background = '#fffbea';
    this.alertEl.style.color = '#7c2d12';
    this.alertEl.style.border = '1px solid #f6e05e';
    this.alertEl.style.borderRadius = '8px';
    this.alertEl.style.padding = '10px 14px';
    this.alertEl.style.boxShadow = '0 6px 20px rgba(0,0,0,0.15)';
    // One-time bind for close button
    if (!this._alertBound) {
        this.alertEl.addEventListener('click', (e) => {
            const closeBtn = e.target.closest('.app-alert__close');
            if (closeBtn) {
                this.hideAlert();
            }
        });
        this._alertBound = true;
    }
    try { console.info('[Alert]', message); } catch (_) {}
...
// ===== Centralized confirm dialog (non-blocking) =====
UIManager.prototype.showConfirm = function(message, { confirmText = 'Confirm', cancelText = 'Cancel' } = {}) {
    // Create container if missing
    if (!this.confirmEl) {
        const el = document.createElement('div');
        el.setAttribute('data-testid', 'app-confirm');
    ...
    // Render minimal modal structure with inline styles to guarantee visibility
    const safeMsg = String(message || 'Are you sure?');
    this.confirmEl.innerHTML = `
      <div class="app-confirm__backdrop"></div>
      <div class="app-confirm__dialog" role="document">
        <div class="app-confirm__content">
          <div class="app-confirm__message"></div>
          <div class="app-confirm__actions">
            <button class="btn btn-secondary app-confirm__cancel" data-testid="confirm-cancel">${cancelText}</button>
            <button class="btn btn-primary app-confirm__ok" data-testid="confirm-ok">${confirmText}</button>
          </div>
        </div>
      </div>`;

    // Inline critical styles (backdrop + dialog)
    const s = this.confirmEl.style; s.display = 'block'; s.position = 'fixed'; s.left = '0'; s.top = '0'; s.width = '100%'; s.height = '100%'; s.zIndex = '9999';
    const backdrop = this.confirmEl.querySelector('.app-confirm__backdrop');
    const dialog = this.confirmEl.querySelector('.app-confirm__dialog');
    const msgEl = this.confirmEl.querySelector('.app-confirm__message');
    if (backdrop) { backdrop.style.position = 'absolute'; backdrop.style.inset = '0'; backdrop.style.background = 'rgba(0,0,0,0.5)'; }
    if (dialog) { dialog.style.position = 'absolute'; dialog.style.top = '50%'; dialog.style.left = '50%'; dialog.style.transform = 'translate(-50%, -50%)'; dialog.style.background = '#fff'; dialog.style.borderRadius = '12px'; dialog.style.minWidth = '280px'; dialog.style.maxWidth = '90%'; dialog.style.boxShadow = '0 20px 60px rgba(0,0,0,0.3)'; }
    const content = this.confirmEl.querySelector('.app-confirm__content');
    if (content) { content.style.padding = '16px'; }
    if (msgEl) { msgEl.textContent = safeMsg; msgEl.style.marginBottom = '12px'; msgEl.style.fontWeight = '600'; msgEl.style.color = '#2d3748'; }
    const actions = this.confirmEl.querySelector('.app-confirm__actions');
    if (actions) { actions.style.display = 'flex'; actions.style.gap = '8px'; actions.style.justifyContent = 'flex-end'; }

    // Promise that resolves on user choice
    return new Promise((resolve) => {
        ...
        // Click handlers
        const okBtn = this.confirmEl.querySelector('.app-confirm__ok');
        const cancelBtn = this.confirmEl.querySelector('.app-confirm__cancel');
        const onKey = (e) => { if (e.key === 'Escape') onResolve(false); };
        okBtn?.addEventListener('click', () => onResolve(true), { once: true });
        cancelBtn?.addEventListener('click', () => onResolve(false), { once: true });
        this.confirmEl.addEventListener('click', (e) => { if (e.target.classList.contains('app-confirm__backdrop')) onResolve(false); }, { once: true });
        document.addEventListener('keydown', onKey, { once: true });
        this._confirmKeyHandler = onKey;
    });
};

UIManager.prototype.hideConfirm = function() {
    if (this.confirmEl) {
...
class FPLTeamManager {
    ...
    // Centralized notifier: uses UI alert container when available, falls back to native alert in tests/mocks
    _notify(message) {
    ...
    openModal(playerId = null) {
        if (this._isReadOnlyCurrentWeek()) {
            this._notify('This week is read-only. Create a new week to make changes.');
            return;
        }
    ...
    handleFormSubmit(e) {
        ...
        if (playerData.have && !this.currentEditingId) {
            ...
            if (currentTeamSize >= 15) {
                this._notify('You can only have 15 players in your team. Please remove a player first.');
                return;
            }
    ...
    addPlayer(playerData) {
        if (this._isReadOnlyCurrentWeek()) {
            this._notify('This week is read-only. Create a new week to add players.');
            return;
        }
        const player = {
    ...
    updatePlayer(playerId, playerData) {
        if (this._isReadOnlyCurrentWeek()) {
            this._notify('This week is read-only. Create a new week to edit players.');
            return;
        }
    ...
    async deletePlayer(playerId) {
        if (this._isReadOnlyCurrentWeek()) {
            this._notify('This week is read-only. Create a new week to delete players.');
            return;
        }
        let confirmed = false;
        const useCustom = typeof window !== 'undefined' && window.__APP_USE_CUSTOM_CONFIRM__ === true;
        if (useCustom && this.ui && typeof this.ui.showConfirm === 'function') {
            confirmed = await this.ui.showConfirm('Are you sure you want to delete this player?', { confirmText: 'Delete', cancelText: 'Cancel' });
        } else if (typeof confirm === 'function') {
            confirmed = confirm('Are you sure you want to delete this player?');
        }
        if (confirmed) {
    ...
    setCaptain(playerId) {
        if (this._isReadOnlyCurrentWeek()) {
            this._notify('This week is read-only. Create a new week to change captain.');
            return;
        }

        const player = this.players.find(p => p.id === playerId);
        if (!player || !player.have) {
            this._notify('Only players in your team can be selected as captain.');
            return;
        }
        if (this.captain === playerId) {
    ...
    setViceCaptain(playerId) {
        if (this._isReadOnlyCurrentWeek()) {
            this._notify('This week is read-only. Create a new week to change vice captain.');
            return;
        }

        const player = this.players.find(p => p.id === playerId);
        if (!player || !player.have) {
            this._notify('Only players in your team can be selected as vice-captain.');
            return;
        }
        if (this.viceCaptain === playerId) {
    ...
    toggleHave(playerId) {
        if (this._isReadOnlyCurrentWeek()) {
            this._notify('This week is read-only. Create a new week to change team membership.');
            return;
        }
        const player = this.players.find(p => p.id === playerId);
        if (!player) return;
        ...
        if (player.have) {
            ...
            if (currentTeamSize >= 15) {
                this._notify('You can only have 15 players in your team. Please remove a player first.');
                return; // Prevent UI re-render from dismissing the alert
            }
            player.have = true;
        }
    ...
    getFilteredPlayers() {
        ...
        // Filter by position - safely check if element exists
        if (this.positionFilter && this.positionFilter.value !== 'all') {
            filteredPlayers = filteredPlayers.filter(player => player.position === this.positionFilter.value);
        }
        
        // Filter by "Have" status - safely check if element exists
        if (this.haveFilter && this.haveFilter.checked) {
    ...
    async goToWeek(weekNumber) {
        ...
        if (weekNumber >= 1 && weekNumber <= totalWeeks) {
            ...
            await this.loadStateFromStorage(); // Reloads state for the target week
            this.updateDisplay();
        }
    }

    async prevWeek() {
        await this.goToWeek(this.currentWeek - 1);
    }
...
document.addEventListener('DOMContentLoaded', () => {
    // Enable custom non-blocking confirm in real browser runtime (avoid enabling in Jest/jsdom)
    const ua = (window.navigator && window.navigator.userAgent) || '';
    if (!/jsdom/i.test(ua)) {
        window.__APP_USE_CUSTOM_CONFIRM__ = true;
    }
...
// Export classes for Node.js/Jest
if (typeof module !== 'undefined' && module.exports) {
    module.exports = { FPLTeamManager, UIManager, StorageService };
}

// Also make available globally for tests
if (typeof global !== 'undefined') {
    global.FPLTeamManager = FPLTeamManager;
