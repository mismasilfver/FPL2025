...
>
> **This project**: `server/database.js` uses `better-sqlite3` with `db.prepare(...).run(...)` / `.get(...)` / `.all(...)` — this is already the parameterized-query pattern. When reviewing changes to that file (or `server/routes/storage.js`), verify new queries follow the same `?`-placeholder pattern rather than string-concatenating request data into SQL.
...
### Node.js (better-sqlite3 / node-postgres)
...
// ✅ better-sqlite3 parameterized (?, ?, ...) — this project's pattern
const row = db.prepare("SELECT * FROM meta WHERE key = ?").get(key);
db.prepare("INSERT INTO weeks (week_number, payload) VALUES (?, ?)").run(weekNumber, payload);
