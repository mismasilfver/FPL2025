...
- Vanilla DOM (no framework): nothing auto-escapes. Audit every `innerHTML`/`outerHTML` assignment; prefer `textContent` or explicit element creation (`createElement` + `setAttribute`/`textContent`).
