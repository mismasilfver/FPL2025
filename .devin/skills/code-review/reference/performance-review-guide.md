...
> **Note for vanilla-DOM code (no framework):** the same principle applies without a framework — avoid re-rendering an entire `<table>`/list from scratch on every state change (e.g. `innerHTML = ''` + rebuild) when only a few rows changed. Prefer diffing/patching specific rows, or at minimum batch DOM writes with `DocumentFragment` before a single append.
...
```markdown
- [ ] useEffect 是否都有清理函数？（vanilla JS：事件监听是否配对 removeEventListener？）
- [ ] 事件监听是否在组件卸载时移除？
- [ ] 定时器是否被清理？
